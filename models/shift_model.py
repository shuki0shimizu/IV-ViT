...
class Attention(nn.Module):
    ...
    def __init__(self, config, vis, shift_type):
    ...
    def shift_attn(self, query_layer, key_layer, is_video):

        b, h, n, d = query_layer.size()
        bs = b // self.t

        query_layer = query_layer.view(bs, self.t, h, n, d)
        key_layer = key_layer.view(bs, self.t, h, n, d)
        Q = torch.zeros_like(query_layer)  # B, t, h, N+1, D/h
        K = torch.zeros_like(query_layer)  # B, t, h, N+1, D/h

        Q[:, :-1, 0:1, :, :] = query_layer[:, 1:, 0:1, :, :]
        Q[:, 1:, 1:2, :, :] = query_layer[:, :-1, 1:2, :, :]
        Q[:, :, 2:12, :, :] = query_layer[:, :, 2:12, :, :]

        K[:, :-1, 0:1, :, :] = key_layer[:, 1:, 0:1, :, :]
        K[:, 1:, 1:2, :, :] = key_layer[:, :-1, 1:2, :, :]
        K[:, :, 2:12, :, :] = key_layer[:, :, 2:12, :, :]

        # move heads:8
        # Q[:, :-1, 0:4, :, :] = query_layer[:, 1:, 0:4, :, :]
        # Q[:, 1:, 4:8, :, :] = query_layer[:, :-1, 4:8, :, :]
        # Q[:, :, 8:12, :, :] = query_layer[:, :, 8:12, :, :]

        # K[:, :-1, 0:4, :, :] = key_layer[:, 1:, 0:4, :, :]
        # K[:, 1:, 4:8, :, :] = key_layer[:, :-1, 4:8, :, :]
        # K[:, :, 8:12, :, :] = key_layer[:, :, 8:12, :, :]
    ...
    def forward(self, hidden_states, is_video):
        ...
        if self.shift_type == "MSCA" and is_video:
            Q, K = self.shift_attn(query_layer, key_layer, is_video)
...
class Block(nn.Module):
    ...
    def __init__(self, config, vis, shift_type):
        ...
        self.ffn = Mlp(config)
        self.attn = Attention(config, vis, shift_type)
    ...
    def forward(self, x, is_video):
        ...
        if self.shift_type == "tokenshift" and is_video:
            x = self.shift_tk(x)
        h = x
        x = self.attention_norm(x)
        x, weights = self.attn(x, is_video)
        x = x + h
...
class Encoder(nn.Module):
    ...
    def __init__(self, config, vis, shift_type):
        ...
        for _ in range(config.transformer["num_layers"]):
            layer = Block(config, vis, shift_type)
            self.layer.append(copy.deepcopy(layer))

    def forward(self, hidden_states, is_video):
        attn_weights = []
        for layer_block in self.layer:
            hidden_states, weights = layer_block(hidden_states, is_video)
            if self.vis:
...
class Transformer(nn.Module):
    ...
    def __init__(self, config, img_size, vis, shift_type):
        super(Transformer, self).__init__()
        self.embeddings = Embeddings(config, img_size=img_size)
        self.encoder = Encoder(config, vis, shift_type)

    def forward(self, input_ids, is_video):
        embedding_output = self.embeddings(input_ids)
        encoded, attn_weights = self.encoder(embedding_output, is_video)
        return encoded, attn_weights
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, config, shift_type, img_size=224, num_classes=400, zero_head=False, vis=False):
        super(VisionTransformer, self).__init__()
        self.num_classes = num_classes
        self.zero_head = zero_head
        self.classifier = config.classifier

        self.transformer = Transformer(config, img_size, vis, shift_type)

        self.head = Linear(config.hidden_size, num_classes)

    def forward(self, x, domain, is_video, labels=None):
        x, attn_weights = self.transformer(x, is_video)
        logits = self.head(x[:, 0], domain)
