...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        self.pos_embedding = nn.Parameter(
            torch.empty(
                1, seq_length, hidden_dim).normal_(
                std=0.02))  # from BERT
        self.dropout = nn.Dropout(dropout)
...
class VisionTransformer(nn.Module):
    ...
    def _process_input(self, x: torch.Tensor, domain=None) -> torch.Tensor:
        n, c, h, w = x.shape
        p = self.patch_size
    ...
    def forward(self, x: torch.Tensor, domain=None):

        # Reshape and permute the input tensor
        if domain is not None:
            x = self._process_input(x, domain)
        else:
...
def vit_b_16(args, **kwargs: Any) -> VisionTransformer:
    """
...
def interpolate_embeddings(
    ...
    # an interpolation in the (h, w) space and then reshaping back to a 1d
    # grid.
    if new_seq_length != seq_length:
        # The class token embedding shouldn't be interpolated so we split it
        # up.
        seq_length -= 1
        new_seq_length -= 1
...
if __name__ == "__main__":
    model = vit_b_16(num_classes=400)
    bs = 2
